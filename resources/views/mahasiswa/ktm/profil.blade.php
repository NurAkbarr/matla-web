                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-8">
                    ...
                    <form action="{{ route('mahasiswa.profil.update') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        ...
                        <div class="text-center mb-6">
                            <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3">Update Foto Profil</label>
                            <div class="relative inline-block group">
                                <img id="preview-foto" src="{{ $user->foto_profil }}" class="w-32 h-32 sm:w-40 sm:h-40 object-cover rounded-full mx-auto border-4 border-white shadow-xl group-hover:opacity-75 transition-all">
                                <label for="input-foto" class="absolute bottom-0 right-0 bg-primary text-white p-2.5 rounded-full shadow-lg cursor-pointer hover:scale-110 active:scale-95 transition-all border-4 border-white">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                                </label>
                                <input type="file" id="input-foto" name="foto" accept="image/*" class="hidden" onchange="previewImage(this)">
                            </div>
                            @error('foto')
                                <p class="text-red-500 text-[10px] font-bold mt-2">{{ $message }}</p>
                            @else
                                <p class="text-[10px] text-gray-400 font-medium mt-2 italic">* Format: JPG, PNG, Max 2MB</p>
                            @enderror
                        </div>
