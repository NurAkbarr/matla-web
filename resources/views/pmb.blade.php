            ...
            @forelse($brosurs as $brosur)
                <div class="w-full max-w-2xl brochure-card-wrapper" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                    <div class="brochure-card group relative bg-white rounded-[2.5rem] p-4 md:p-6 shadow-2xl border border-gray-100 transition-all duration-500 hover:shadow-primary/10 h-full flex flex-col cursor-pointer" 
                        ...
                        <!-- Premium Frame - Anti Gepeng (Natural Ratio) -->
                        <div class="relative bg-gray-50 rounded-[2rem] mb-8 flex-shrink-0 border border-gray-100 flex items-center justify-center p-2 overflow-hidden" style="height: 75vh;">
                            <img src="{{ asset('pmb-brosur/' . $brosur->image) }}" alt="{{ $brosur->title }}" class="max-w-full max-h-full w-auto h-auto object-contain rounded-xl shadow-sm transform group-hover:scale-[1.01] transition-transform duration-700 mx-auto block">
