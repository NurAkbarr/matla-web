    ...
    <div class="flex h-screen bg-gray-50 font-sans text-gray-900 overflow-hidden w-full">
        ...
        <aside id="sidebar" 
            ...
            <nav class="flex-1 overflow-y-auto scrollbar-hide py-6 px-4 space-y-1">
                ...
                {{-- PRIVATE (FOR DEVELOPER ONLY) --}}
                <p class="px-4 pt-8 pb-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Private</p>
                <a href="{{ route('backend.admin.finance.login') }}" 
                   class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all group {{ request()->routeIs('backend.admin.finance.*') ? 'bg-primary text-white shadow-md shadow-primary/20' : 'text-slate-500 hover:bg-slate-50 hover:text-primary' }}">
                    <svg class="w-5 h-5 {{ request()->routeIs('backend.admin.finance.*') ? 'text-white' : 'text-slate-400 group-hover:text-primary' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                    </svg>
                    <span class="text-sm font-bold">Manajemen Keuangan</span>
                </a>
                @endif
