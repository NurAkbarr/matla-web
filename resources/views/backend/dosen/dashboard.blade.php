...
<body class="text-slate-800 antialiased" x-data="{ sidebarOpen: false, mobileMenuOpen: false }">
    ...
    <aside class="fixed inset-y-0 left-0 bg-white shadow-xl w-72 z-50 transform transition-transform duration-300 flex flex-col"
        ...
        <div class="flex-1 overflow-y-auto py-5 px-4 space-y-6 no-scrollbar">
            ...
            <a href="{{ route('backend.dosen.dashboard') }}" class="flex items-center gap-3 px-4 py-3.5 rounded-xl sidebar-active shadow-lg shadow-emerald-700/20">
            ...
            <div>
                ...
                <nav class="space-y-1">
                    <a href="{{ route('backend.dosen.jadwal') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-600 hover:bg-emerald-50 hover:text-emerald-700 transition-colors">
                        <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>
                        <span class="font-semibold text-[13px]">Jadwal</span>
                    </a>
                    <a href="{{ route('backend.dosen.presensi.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-600 hover:bg-emerald-50 hover:text-emerald-700 transition-colors">
                        ...
                        <span class="font-semibold text-[13px]">Absen Mengajar</span>
                    </a>
                    <a href="{{ route('backend.dosen.nilai.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-600 hover:bg-emerald-50 hover:text-emerald-700 transition-colors">
                        <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                        <span class="font-semibold text-[13px]">Input Nilai</span>
                    </a>
                    <a href="{{ route('backend.admin.assignments.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-600 hover:bg-emerald-50 hover:text-emerald-700 transition-colors">
                        <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg>
                        <span class="font-semibold text-[13px]">Tugas MHS</span>
                    </a>
                    <a href="{{ route('backend.dosen.profile.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-600 hover:bg-emerald-50 hover:text-emerald-700 transition-colors">
        ...
        <div class="p-4 border-t border-slate-100 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <img src="{{ Auth::user()->avatar_url ?? 'https://ui-avatars.com/api/?name='.urlencode(Auth::user()->name) }}" class="w-10 h-10 rounded-xl border border-slate-200 shadow-sm object-cover" alt="Profile">
                <div>
                    <p class="text-[13px] font-bold text-slate-900 leading-tight">{{ Auth::user()->name }}</p>
                    <p class="text-[11px] text-slate-500 font-medium">Dosen Tetap</p>
                </div>
            </div>
    ...
    <div class="lg:ml-72 min-h-screen flex flex-col transition-all duration-300">
        ...
        <header class="h-[76px] bg-white border-b border-slate-100 flex items-center justify-between px-6 lg:px-8 sticky top-0 z-30">
            <div class="flex items-center gap-4">
                ...
                <h2 class="text-xl font-bold text-slate-800 tracking-tight">Dashboard Dosen</h2>
            </div>
            ...
            <div class="flex items-center gap-4 lg:gap-6">
                ...
                <div class="relative pl-4 lg:pl-6 border-l border-slate-200" x-data="{ profileOpen: false }">
                    <button @click="profileOpen = !profileOpen" @click.away="profileOpen = false" class="flex items-center gap-3 text-left focus:outline-none group">
                        ...
                        <img src="{{ Auth::user()->avatar_url ?? 'https://ui-avatars.com/api/?name='.urlencode(Auth::user()->name) }}" class="w-9 h-9 rounded-full border-2 border-white shadow-sm object-cover ring-2 ring-transparent group-hover:ring-emerald-500 transition-all" alt="Profile">
                    </button>
                    ...
                    <div x-show="profileOpen" 
                        ...
                        <div class="px-5 pb-4 border-b border-slate-100 mb-2">
                            ...
                            <div class="flex items-center gap-2 mt-3">
                                <span class="px-2 py-1 bg-slate-100 text-slate-600 rounded-md text-[10px] font-bold">Dosen</span>
                                <span class="px-2 py-1 bg-emerald-50 text-emerald-600 rounded-md text-[10px] font-bold">NIDN: {{ Auth::user()->nidn ?? '-' }}</span>
                            </div>
                        </div>

                        <a href="{{ route('backend.dosen.profile.index') }}" class="flex items-center px-5 py-2.5 text-sm text-slate-600 hover:text-emerald-700 hover:bg-emerald-50 transition-colors">
            ...
            <div class="max-w-7xl mx-auto">
                    ...
                    <div class="relative z-10">
                        <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-10">
                            <div class="max-w-2xl">
                                ...
                                <p class="text-emerald-100 text-sm opacity-90">Semoga harimu menyenangkan. Berikut ringkasan aktivitas akademikmu hari ini.</p>
                            </div>
                            <div class="flex gap-4">
                                <div class="rounded-2xl border border-white/20 bg-white/10 p-4 min-w-[120px] text-center backdrop-blur-sm">
                                    ...
                                    <p class="text-lg font-bold mt-2 uppercase tracking-wide">Aktif Mengajar</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-5">
                            <div class="rounded-[1.5rem] bg-emerald-600 border border-emerald-500/50 p-6 shadow-inner text-white flex items-center justify-between">
                                <div>
                                    <p class="text-[11px] uppercase tracking-[0.2em] font-bold text-emerald-100 mb-2">Total Kelas</p>
                                    <h3 class="text-5xl font-bold mb-1">{{ $totalKelas }}</h3>
                                </div>
                                <div class="w-16 h-16 bg-emerald-500/50 rounded-2xl flex items-center justify-center">
                                    <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                                    </svg>
                                </div>
                            </div>
                            <div class="rounded-[1.5rem] bg-white p-6 shadow-md border border-slate-100 text-slate-800 flex items-center justify-between">
                                <div>
                                    <p class="text-[11px] uppercase tracking-[0.2em] font-bold text-slate-400 mb-2">Total Mahasiswa Ajar</p>
                                    <h3 class="text-5xl font-bold mb-1">{{ $totalMahasiswa }}</h3>
                                </div>
                ...
                <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-200">
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <!-- Jadwal -->
                        <a href="{{ route('backend.dosen.jadwal') }}" class="flex flex-col items-center p-5 rounded-[1.5rem] border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50/50 hover:shadow-md transition-all group">
                            ...
                            <span class="font-bold text-[13px] text-slate-800 group-hover:text-emerald-700">Jadwal Mengajar</span>
                        </a>
                        <!-- Absen -->
                        <a href="{{ route('backend.dosen.presensi.index') }}" class="flex flex-col items-center p-5 rounded-[1.5rem] border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50/50 hover:shadow-md transition-all group">
                            <div class="w-12 h-12 rounded-2xl bg-emerald-50 text-emerald-700 flex items-center justify-center mb-4 group-hover:bg-emerald-600 group-hover:text-white transition-colors">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                            </div>
                            <span class="font-bold text-[13px] text-slate-800 group-hover:text-emerald-700">Absen Mengajar</span>
                        </a>
                        <!-- Nilai -->
                        <a href="{{ route('backend.dosen.nilai.index') }}" class="flex flex-col items-center p-5 rounded-[1.5rem] border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50/50 hover:shadow-md transition-all group">
                            <div class="w-12 h-12 rounded-2xl bg-emerald-50 text-emerald-700 flex items-center justify-center mb-4 group-hover:bg-emerald-600 group-hover:text-white transition-colors">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                            </div>
                            <span class="font-bold text-[13px] text-slate-800 group-hover:text-emerald-700">Input Nilai</span>
                        </a>
                        <!-- Tugas MHS -->
                        <a href="{{ route('backend.admin.assignments.index') }}" class="flex flex-col items-center p-5 rounded-[1.5rem] border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50/50 hover:shadow-md transition-all group">
                            <div class="w-12 h-12 rounded-2xl bg-emerald-50 text-emerald-700 flex items-center justify-center mb-4 group-hover:bg-emerald-600 group-hover:text-white transition-colors">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg>
                            </div>
                            <span class="font-bold text-[13px] text-slate-800 group-hover:text-emerald-700">Tugas Mahasiswa</span>
                        </a>
                    </div>
                </div>
