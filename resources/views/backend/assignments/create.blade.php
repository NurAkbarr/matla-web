...
<div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="flex items-center space-x-4 bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
        ...
        <div>
            <h1 class="text-3xl font-black text-slate-800 tracking-tight">Buat Tugas MHS</h1>
            <p class="text-slate-500 font-medium mt-1">Publikasikan penugasan baru lengkap dengan berkas dan tautan referensi.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden p-10">
        <form action="{{ route('backend.admin.assignments.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="title" id="title" required placeholder="Contoh: Makalah Analisis Pendidikan Kontemporer" value="{{ old('title') }}" class="w-full px-5 py-4 bg-slate-50 border border-slate-200 focus:border-primary focus:bg-white rounded-2xl text-slate-700 font-medium transition-all outline-none">
                @error('title')
                    <p class="text-xs text-rose-500 font-bold mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" id="description" rows="6" placeholder="Masukkan detail penugasan, sistematika penulisan, atau kriteria penilaian..." class="w-full px-5 py-4 bg-slate-50 border border-slate-200 focus:border-primary focus:bg-white rounded-2xl text-slate-700 font-medium transition-all outline-none resize-none">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-xs text-rose-500 font-bold mt-1">{{ $message }}</p>
                @enderror
            </div>
                    ...
                    <div class="relative flex items-center justify-center w-full">
                        <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-200 hover:border-primary rounded-2xl cursor-pointer hover:bg-slate-50 transition-all">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                ...
                                <p class="text-xs text-slate-400 font-bold">Klik untuk unggah berkas (Max 10MB)</p>
                                <p id="file-chosen" class="text-xs text-primary font-bold mt-1 hidden"></p>
                            </div>
