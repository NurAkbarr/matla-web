...
@section('breadcrumb', 'Akademik / Program Studi / Edit')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-8 border-b border-gray-50">
            <h3 class="text-xl font-extrabold text-gray-900">Edit Program Studi</h3>
            <p class="text-sm text-gray-500 mt-1">Perbarui informasi program akademik: <span class="font-bold text-primary">{{ $programStudi->nama }}</span></p>
        </div>

        <form action="{{ route('backend.admin.program-studi.update', $programStudi) }}" method="POST" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2 md:col-span-2">
                    ...
                    <input type="text" name="nama" id="nama" required
                        ...
                        placeholder="Contoh: Teknik Informatika" value="{{ old('nama', $programStudi->nama) }}">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="singkatan" id="singkatan" required
                        ...
                        placeholder="Contoh: TI" value="{{ old('singkatan', $programStudi->singkatan) }}">
                    ...
                    <div class="relative">
                        <select name="akreditasi" id="akreditasi"
                            ...
                            @foreach(['Unggul','Baik Sekali','Baik','B','C'] as $akr)
                                <option value="{{ $akr }}" {{ old('akreditasi', $programStudi->akreditasi) == $akr ? 'selected' : '' }}>{{ $akr }}</option>
                            @endforeach
                        </select>
                        <div class="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-gray-400">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                ...
                <div class="space-y-2 md:col-span-2">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4"
                        ...
                        placeholder="Deskripsi singkat tentang program studi ini...">{{ old('deskripsi', $programStudi->deskripsi) }}</textarea>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" name="urutan" id="urutan" min="0"
                        ...
                        value="{{ old('urutan', $programStudi->urutan) }}">
                </div>
                ...
                <div class="space-y-2 flex flex-col justify-end">
                    ...
                    <label for="is_active" class="flex items-center space-x-3 px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl cursor-pointer hover:border-primary/30 transition-all">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $programStudi->is_active) ? 'checked' : '' }}
