...
@section('breadcrumb', 'Akademik / Jadwal / Edit')

@section('content')
<div class="max-w-4xl bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
    <div class="p-8 border-b border-gray-50 bg-gray-50/50">
        <h3 class="text-xl font-extrabold text-gray-900">Perbarui Jadwal Perkuliahan</h3>
        <p class="text-sm text-gray-500 font-medium">Ubah informasi jadwal mata kuliah di bawah ini.</p>
    </div>

    <form action="{{ route('backend.admin.jadwal.update', $jadwal) }}" method="POST" class="p-8 space-y-8">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-3">
                ...
                <select name="program_studi_id" id="program_studi_id" required 
                    ...
                    @foreach($programStudis as $prodi)
                        <option value="{{ $prodi->id }}" {{ old('program_studi_id', $jadwal->program_studi_id) == $prodi->id ? 'selected' : '' }}>{{ $prodi->nama }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="space-y-3 border-t border-gray-50 pt-8">
            ...
            <input type="text" name="ruang" id="ruang" required 
                class="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all text-sm font-bold" 
                placeholder="Contoh: Ruang Galon atau Ruang A-101" value="{{ old('ruang', $jadwal->ruang) }}">
        </div>
