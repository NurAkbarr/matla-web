...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
    ...
    <div class="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-gray-100 group hover:shadow-xl transition-all duration-500">
        ...
        <div class="p-4 md:p-6">
            <div class="mb-3 md:mb-4">
                <span class="text-[9px] md:text-[10px] text-gray-400 font-bold uppercase tracking-widest">Order: #{{ $brosur->order }}</span>
                <h3 class="text-base md:text-xl font-bold text-gray-900 group-hover:text-primary transition-colors leading-snug">{{ $brosur->title }}</h3>
                <p class="text-xs md:text-sm text-gray-500 mt-1 md:mt-2 line-clamp-2">{{ $brosur->description ?? 'Tidak ada deskripsi.' }}</p>
            </div>

            <div class="flex items-center justify-between pt-3 md:pt-4 border-t border-gray-50">
                <div class="flex space-x-1.5 md:space-x-2">
                    <a href="{{ route('backend.admin.pmb.brosur.edit', $brosur->id) }}" class="p-1.5 md:p-2 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-lg md:rounded-xl transition-all">
                ...
                <a href="{{ asset('pmb-brosur/' . $brosur->image) }}" download class="text-[10px] md:text-xs font-bold text-primary hover:underline flex items-center space-x-1">
    ...
    <div class="col-span-full py-16 md:py-20 text-center bg-white rounded-[2rem] border border-dashed border-gray-200">
        ...
        <p class="text-gray-400 font-medium italic text-sm">Belum ada brosur yang ditambahkan.</p>
    </div>
    @endforelse
</div>
@endsection
