...
<div class="max-w-4xl mx-auto">
    <div class="mb-6 md:mb-10">
        <h1 class="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight">Edit Brosur</h1>
        <p class="text-xs md:text-sm text-gray-500 font-medium italic">Perbarui informasi atau file brosur Anda</p>
    </div>
        ...
        <form action="{{ route('backend.admin.pmb.brosur.update', $brosur->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-5 md:space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    @if($brosur->image)
                    <div class="w-24 md:w-32 aspect-[3/4] rounded-xl md:rounded-2xl overflow-hidden border-4 border-white shadow-lg mb-4">
                    ...
                    <div class="border-2 border-dashed border-gray-200 rounded-xl md:rounded-2xl p-4 md:p-6 text-center hover:border-primary transition-colors bg-gray-50/50">
                        <input type="file" name="image" accept="image/*" class="w-full text-xs md:text-sm text-gray-500 file:mr-3 md:file:mr-4 file:py-2 md:file:py-3 file:px-4 md:file:px-6 file:rounded-full file:border-0 file:text-[10px] md:file:text-xs file:font-bold file:bg-primary/10 file:text-primary hover:file:bg-primary/20 cursor-pointer">
                        <p class="text-[10px] md:text-xs text-gray-400 mt-3 md:mt-4 font-medium italic">Kosongkan jika tidak ingin mengubah. Format: JPEG, PNG, WEBP (Max 5MB).</p>
                    </div>
