...
<div class="mb-6 md:mb-10">
    ...
    <p class="text-xs md:text-sm text-gray-500 font-medium italic ml-12">Unggah gambar dan tautkan ke Instagram</p>
</div>
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                @error('instagram_link')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="text-xs text-gray-400 mt-2 italic">Copy link langsung dari postingan Instagram yang bersangkutan.</p>
            </div>

            <!-- File -->
            <div>
                <label for="image" class="block text-sm font-bold text-gray-700 mb-2">File Foto/Video <span class="text-red-500">*</span></label>
                <input type="file" name="image" id="image" class="w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20 transition-colors" accept="image/*,video/mp4,video/quicktime" required>
                @error('image')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="text-xs text-gray-400 mt-2 italic">Format Foto: JPG, PNG. Format Video: MP4, MOV. Maks 20MB.</p>
            </div>

            <!-- Caption -->
            <div>
                <label for="caption" class="block text-sm font-bold text-gray-700 mb-2">Caption / Deskripsi (Opsional)</label>
                <textarea name="caption" id="caption" rows="4" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary/20 transition-colors" placeholder="Ketik caption lengkap di sini...">{{ old('caption') }}</textarea>
                @error('caption')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-100">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-2 italic">Angka terkecil akan tampil paling depan.</p>
                </div>
